...
static uint16_t ICM20948_read_from_double_register(I2C_HandleTypeDef* i2c, uint8_t high_address, uint8_t low_address){
    uint16_t data;
...
//    HAL_I2C_Master_Transmit_IT(i2c, ICM20948_ADDR<<1, &address, sizeof(address));
//    HAL_Delay(100);
//    HAL_I2C_Master_Receive_IT(i2c, ICM20948_ADDR<<1, data->raw_data, 1);
//    volatile HAL_StatusTypeDef status = HAL_I2C_Mem_Read_DMA(i2c, ICM20948_ADDR<<1, ACCEL_XOUT_H, 12, data->raw_data, 12);
//    HAL_Delay(1);
//
//
//
//    data->accel[0] = (int16_t)(data->raw_data[0] << 8 | data->raw_data[1]);
//    data->accel[1] = (int16_t)(data->raw_data[2] << 8 | data->raw_data[3]);
//    data->accel[2] = (int16_t)(data->raw_data[4] << 8 | data->raw_data[5]);
    ...
    data->gyro[0] = (int16_t)ICM20948_read_from_double_register(i2c, GYRO_XOUT_H, GYRO_XOUT_L) / 131.f;
    data->gyro[1] = (int16_t)ICM20948_read_from_double_register(i2c, GYRO_YOUT_H, GYRO_YOUT_L) / 131.f;
    data->gyro[2] = (int16_t)ICM20948_read_from_double_register(i2c, GYRO_ZOUT_H, GYRO_ZOUT_L) / 131.f;
    return 0;
}
